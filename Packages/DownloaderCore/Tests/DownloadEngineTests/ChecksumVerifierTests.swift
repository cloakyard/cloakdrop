...
struct ChecksumVerifierTests {
    ...
    func knownDigests() throws {
        let url = try writeTempFile("hello")
        defer { try? FileManager.default.removeItem(at: url) }

        #expect(try ChecksumVerifier.hash(fileURL: url, algorithm: .md5) == "5d41402abc4b2a76b9719d911017c592")
        #expect(try ChecksumVerifier.hash(fileURL: url, algorithm: .sha1) == "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d")
        #expect(try ChecksumVerifier.hash(fileURL: url, algorithm: .sha256) == "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824")
    }
    ...
    func verifyMatch() throws {
        ...
        #expect(throws: Never.self) { try ChecksumVerifier.verify(fileURL: url, against: good) }

        let bad = ChecksumExpectation(algorithm: .sha256, expectedHex: String(repeating: "0", count: 64))
        #expect(throws: DownloadError.self) { try ChecksumVerifier.verify(fileURL: url, against: bad) }
    }
    ...
    func chunkInvariance() throws {
        ...
        let whole = try ChecksumVerifier.hash(fileURL: url, algorithm: .sha256, chunkSize: 1 << 20)
        let tiny = try ChecksumVerifier.hash(fileURL: url, algorithm: .sha256, chunkSize: 7)
        #expect(whole == tiny)
    }
}
